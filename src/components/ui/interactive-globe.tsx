...
interface GlobeProps {
    ...
    markers?: { lat: number; lng: number; label?: string; data?: any }[];
...
export function Component({
    ...
    const dragRef = useRef<{
        ...
        startRotX: number;
        isClick: boolean;
    }>({ active: false, startX: 0, startY: 0, startRotY: 0, startRotX: 0, isClick: true });
    const animRef = useRef<number>(0);
    const timeRef = useRef(0);
    const renderedMarkersRef = useRef<{ id: number; x: number; y: number }[]>([]);
    ...
    const draw = useCallback(() => {
        ...
        // Draw markers
        const newRenderedMarkers: { id: number; x: number; y: number }[] = [];
        for (let i = 0; i < markers.length; i++) {
            ...
            newRenderedMarkers.push({ id: i, x: sx, y: sy });
        }

        renderedMarkersRef.current = newRenderedMarkers;
    ...
    const onPointerUp = useCallback((e: React.PointerEvent) => {
        if (dragRef.current.isClick && onMarkerClick) {
            const canvas = canvasRef.current;
            if (canvas) {
                const rect = canvas.getBoundingClientRect();
                // We must use standard pointer coordinates over the canvas CSS size
                ...
                for (const m of renderedMarkersRef.current) {
                ...
                if (closest) {
                    onMarkerClick(markers[closest.id]);
                }
            }
        }
